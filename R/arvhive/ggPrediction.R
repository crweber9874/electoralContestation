#' Generate Winner/Loser Effects Plot with DV Distribution
#'
...
#' the raw distribution of the dependent variable in each facet. It is designed for
#' categorical/ordinal dependent variables.
#'
...
#'   by `dv_col_name` and `facet_col_name`.
...
#' @param title Character string. Main title of the plot. Defaults to `"Winner Loser Effects"`.
...
#' @export
ggWinnerLoserPlot <- function(plot_dat,
                              raw_data_for_rug,
                              dv_col_name = "burn_flag",
                              facet_col_name = "presvote_trump_2020",
                              color_col_name = "prepost",
                              category_levels = c("1", "2", "3", "4", "5"),
                              category_labels = c("Strongly Disagree", "Disagree", "Neither Agree nor Disagree", "Agree", "Strongly Agree"),
                              facet_levels = c("0", "1"),
                              facet_labels = c("Trump Voter", "Biden Voter"),
                              color_levels = c("pre", "post"),
                              color_values = c("grey", "black"),
                              color_labels = c("Pre-Survey", "Post-Survey"), # Added for clearer legend
                              point_size = 3,
                              dodge_width = 0.6,
                              linerange_width = 1.25,
                              title = "Winner Loser Effects",
  ...
  # --- Data Preparation for Main Plot (plot_dat) ---
  # These mutations are applied inside the function for consistency
  plot_dat_prepared <- plot_dat %>%
    mutate(
      ...
      !!sym(color_col_name) := factor(.data[[color_col_name]], levels = color_levels, labels = color_labels),
      # Use dynamic column name for facet_col_name
      !!sym(facet_col_name) := factor(.data[[facet_col_name]], levels = facet_levels, labels = facet_labels)
    )

  # --- Data Preparation for Rug Plot (side_plot) ---
  # Use dynamic column names here as well
  side_plot_prepared <- raw_data_for_rug %>%
    dplyr::select(!!sym(dv_col_name), !!sym(facet_col_name)) %>%
    mutate(
      .category = jitter(as.numeric(.data[[dv_col_name]]), amount = rug_jitter_height * 5), # Jitter more broadly for actual categories
      !!sym(facet_col_name) := factor(.data[[facet_col_name]], levels = facet_levels, labels = facet_labels)
    ) %>%
    drop_na(.category, !!sym(facet_col_name))


  # --- Construct the ggplot ---
  p <- ggplot(plot_dat_prepared,
              aes(x = .category, y = mean, ymin = lower, ymax = upper, colour = .data[[color_col_name]])) +
    facet_wrap(~.data[[facet_col_name]], nrow = 2, ...) + # Use dynamic facet_col_name and pass extra args
    ...
    geom_linerange(aes(x=.category, ymax=mean, ymin=0), linewidth = linerange_width, position = position_dodge(width = dodge_width)) +

    # Add the rug plot (raw DV distribution)
    geom_rug(data = side_plot_prepared,
             aes(x = .category, y = 0, # Map .category to x (pre-flip), and dummy y=0
                 colour = NULL, # Remove inherited color from prepost for rug
                 ymin = NULL, ymax = NULL, group = NULL), # Null other inherited aesthetics
             inherit.aes = FALSE,
             position = position_jitter(width = rug_jitter_height, seed = 123), # Jitter horizontally along categorical axis
             sides = rug_side,
             linewidth = rug_size,
             alpha = rug_alpha,
             color = rug_color,
    ...
    scale_colour_manual(
      name = "", # Empty legend title as in your original
      values = color_values,
      labels = color_labels
    ) +
