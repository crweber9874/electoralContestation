...
ggMargins_nc <- function(data = plotData,
  ...
  if ((".category" %in% names(data))) {
    ...
    plot <-
      ...
      ggplot2::geom_point(size = 2, position = position_dodge(width = 0.5), alpha = point_opacity) +
      ...
      ggplot2::theme(
        plot.title = ggplot2::element_text(size = size_title, hjust = 0, vjust = 0, colour = "#3C3C3C"),
        axis.text  = ggplot2::element_text(size = size_x, colour = "#535353"),  # Simplified axis text
        axis.title = ggplot2::element_text(size = size_title, colour = "#535353"),
