...
summary(fit, fit.measures = TRUE, standardized = TRUE)[[7]] 
...
fit_multi2 <- suppressWarnings(sem(model,
                                 data = dat,
                                # group = "party_identification3",
                                # group.equal = c( "intercepts", "residuals"),
                                 missing = "fiml"))

parameter_estimates <- parameterEstimates(fit_multi2) %>%
  filter((lhs == "f1"| lhs =="f2") &
          (op == "~1"| op== "~" & rhs == "post2020")) %>% print()
