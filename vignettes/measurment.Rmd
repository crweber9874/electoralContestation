...
electoral_contestation %>% 
  # replace attend_march as NA if socialmedia_treat = 1
  mutate(attend_march = ifelse(violent_treat == 1, NA, attend_march)) %>%
  mutate(criticize_election = ifelse(socialmedia_treat == 1, NA, criticize_election))
```
...
runEFA = function(factors = 2, 
         ...
         fit <- suppressWarnings(efa(data = df,
                            nfactors = factors,
           ))
       if(factors == 2) summary(fit)[[10]]$nf2[[1]]$eigvals[[1]]
       if(factors == 3) summary(fit)[[10]]$nf3[[1]]$eigvals[[1]]
       if(factors == 4) summary(fit)[[10]]$nf4[[1]]$eigvals[[1]]
         ...
         # eigenvalues = summary(fit)[[10]]$nf3[[1]]$eigvals[[1]]
         # with four
         # eigenvalues = summary(fit)[[10]]$nf4[[1]]$eigvals[[1]]
...
contestation_names = c("state_certify", "concede","criticize_election", "attend_march", "court",  "recount", "new_election", "undermine", "violent") 


electoral_contestation %>%
  filter(survey == "cesASU") %>% 
  select(all_of(contestation_names))  %>%
  select_if(~ !all(is.na(.))) %>% runEFA(factors = 2)
